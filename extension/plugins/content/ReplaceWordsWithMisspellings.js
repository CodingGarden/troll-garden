...
const replacements = [
  ...
  ['todays', 'today\'s'],
  ['wasnt', 'wasn\'t'],
].reduce((lookup, [misspelling, correctSpellings]) => {
  correctSpellings.split(',').forEach((spelling) => {
    const trimmedSpelling = spelling.trim();
    lookup.set(trimmedSpelling, lookup.get(trimmedSpelling) || []);
    lookup.get(trimmedSpelling).push(misspelling);
  });
  return lookup;
}, new Map());

const replacementRegex = new RegExp([...replacements.keys()].join('|'), 'gi');

class ReplaceWordsWithMisspellings extends ContentPlugin {
  constructor() {
  ...
  run() {
    console.log('Running misspelling replacer...');
    function replaceInText(element) {
      ...
      for (const node of element.childNodes) {
        ...
        switch (node.nodeType) {
          case Node.TEXT_NODE:
            node.textContent = node.textContent.replace(replacementRegex, (match) => {
              const misspellings = replacements.get(match)
                || replacements.get(match.toLowerCase());
              const randomIndex = Math.floor(Math.random() * misspellings.length);
              return misspellings[randomIndex];
            });
            break;
          default:
            replaceInText(node);
...
export default ReplaceWordsWithMisspellings;
